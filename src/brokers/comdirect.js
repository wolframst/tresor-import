...
const findShares = (textArr) => {
  const sharesLine = textArr[textArr.findIndex(t => t.includes('Nennwert')) + 1];
...
const findPrice = (textArr) => {
  const priceLine = textArr[textArr.findIndex(t => t.includes('St.'))];
  const price = priceLine.split('EUR  ')[1];
  // console.log('Preis', price)
  return parseGermanNum(price);
}

const findAmount = (textArr) => {
  const priceArea = textArr.slice(textArr.findIndex(t => t.includes('Kurswert')))
...
export const parseComdirectActivity = (textArr) => {
  const isBuy = textArr.some(t => t.includes('Wertpapierkauf'));
  const isSell = textArr.some(t => t.includes('Wertpapierverkauf'));
  const isDividend = textArr.some(t => t.includes('Ertragsgutschrift')) || textArr.some(t => t.includes('Dividendengutschrift'));
